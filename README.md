# Retail sales KPI Dasboard (SQL + Power BI)

## OVERVIEW
This project analyses over 500k+ retail transactions from a kaggle dataset and builds a KPI and returns risk dashboard.
This project aims to quantify revenue performance and identify returns related revenue losses and identify high returned products.

## TOOLS
- PostgreSQL
- Power BI
- SQL

## DASHBOARD OVERVIEW
-----Page 1-----
- Net revenue, orders, active customers
- Month over Month revenue trend
- Time-based (Monthly) filtering

<img width="1537" height="866" alt="page1-exec overview" src="https://github.com/user-attachments/assets/8bbfac3c-bdd1-4bdf-9c93-06f15f1c9811" />

-----Page 2-----
- Return revenue and return rate as risk indicators
- Return trends over time
- Most returned products by revenue

<img width="1537" height="866" alt="page2-returns risk" src="https://github.com/user-attachments/assets/b6e6a4b0-6a28-472f-9ccf-985051600dc4" />



